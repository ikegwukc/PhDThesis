...
\RC{Made some small changes to hopefully clarify. Verify.}
...
%Maybe Remove SML?
In Chapter \ref{Chapter:SML}, we introduce a programming language agnostic framework called SML that integrates a query-like language to simplify the development of machine learning pipelines.  We show that SML significantly reduces the code one has to write to develop the constituent parts of a machine learning pipeline. In Chapter \ref{Chapter:ABIS}, we utilized random forest trees to predict the annual direction of profitability for firms with a minimal amount of information.  We demonstrate that we can out-perform benchmark models typically used in financial accounting research.  
